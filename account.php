...
echo webv_pmpro_shortcode_account('');


function webv_pmpro_shortcode_account($atts, $content=null, $code="")
{
	global $wpdb, $pmpro_msg, $pmpro_msgt, $pmpro_levels, $current_user, $levels, $days_to_expire;
	...
	<div id="pmpro_account">		
		<?php if(in_array('membership', $sections) || in_array('memberships', $sections)) { ?>
			<div id="pmpro_account-membership" class="pmpro_box" style="border-top: none; margin: 0; padding: 0 0 1em 0;">
				
				<h3><?php _e("Membership Details", "pmpro");?></h3>
				<table width="100%" cellpadding="0" cellspacing="0" border="0">
					<thead>
						<tr>
							<th><?php _e("Membership Status", "pmpro");?></th>
							<th><?php _e(" ", "pmpro"); ?></th>
					...
					<tbody>
						...
						<tr>
									...
									<?php 
										...
										if(count($pmpro_levels) > 1 && !defined("PMPRO_DEFAULT_LEVEL")) { ?>
											...
											<?php } elseif  ( $level->id == '3' ) { ?>
												<a href="<?php echo pmpro_url("levels")?>"><?php _e("Sign up for Work Permit", "pmpro");?></a>
											<?php } else { ?>
												<a href="<?php echo pmpro_url("levels")?>"><?php _e("Renew for Next Year", "pmpro");?></a>
											<?php }
											} ?>
							...
							<td class="pmpro_account-membership-levelfee">
								<p><?php 
										if ( $level->id == '2' ) {
											echo 'Your application is in process';
										} else {
											echo '';
		...
		<?php if  ( $level->id == '3' ) { ?>
			<div style="margin: -20px 0 20px 0">
				<!-- a href="<?php echo pmpro_url("levels")?>"><?php _e("Sign up for Work Permit", "pmpro");?></a -->
				<a href="/membership-account/membership-checkout/?level=4"><?php _e("Sign up for Work Permit", "pmpro");?></a>
			</div> 	
		<?php } ?>
		
		<?php if(in_array('profile', $sections)) { ?>
			<div id="pmpro_account-profile" class="pmpro_box" style="border-top: none; margin: 0; padding: 0 0 1em 0;">	
				<?php wp_get_current_user(); ?>
				<h3><?php _e("Account Details", "pmpro");?></h3>
				<?php if($current_user->user_firstname) { ?>
					<p><strong>Name: </strong><?php echo $current_user->user_firstname?> <?php echo $current_user->user_lastname?></p>
				<?php } ?>
				<ul>
					<?php do_action('pmpro_account_bullets_top');?>
					<li style="list-style: none; margin-left: -15px; line-height: 2m;"><strong><?php _e("Username", "pmpro");?>:</strong> <?php echo $current_user->user_login?></li>
					<li style="list-style: none; margin-left: -15px; line-height: 2em;"><strong><?php _e("Email", "pmpro");?>:</strong> <?php echo $current_user->user_email?></li>
		...
		<!-- DISPLAY OF INVOICES ON MEMBERSHIP PAGE IS DISABLED -->
		...
		<div id="pmpro_account-links" class="pmpro_box" style="border-top: none; margin: 0; padding: 0 0 1em 0;">
...
add_shortcode(‘pmpro_account’, ‘webv_pmpro_shortcode_account’);
